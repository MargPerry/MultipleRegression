...
```
This variable represents the endowment Assets per Student in 2000. The minimum number of endowments is 0.634848, and the maximum is 23857.41. There are 1481 missing variables, and this may cause a problem in our  dataset. A way to get through this is to probably use data imputation methods to account for missing data. 

```{r}
favstats(~count, data=newData)
```
The count variable is the average number of students per cohort. The minimum seems too small, but it could be that there is a lowere graduation rate at those particular colleges. The 26989.67 maximum is reasonable as it represents the very large collegs. There are no missing observations in this case. 
...
>>>>>>> e0dcd8f9f56d549db2e09ebac1a575ba20023119
